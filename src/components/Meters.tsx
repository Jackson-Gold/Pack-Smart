...
export default function Meters() {
  ...
  const maxWeight = containerOrder.reduce((acc,id)=> acc + (containers[id].weightCap || 0), 0) || 1

  return (
    <div style={{display:'flex', alignItems:'center', gap:24}}>
      <div>
        <div style={{fontSize:12, marginBottom:6}}>Weight</div>
        <div className="meter" role="meter" aria-valuemin={0} aria-valuemax={maxWeight} aria-valuenow={totalWeight}>
          <span style={{ width: `${Math.min(100, totalWeight/maxWeight*100)}%` }} />
        </div>
        <small>{totalWeight.toFixed(1)} / {maxWeight} kg (all containers)</small>
      </div>
      <div>
        ...
        <small>{totalCells} / {maxCells} inches</small>
      </div>
    </div>
  )
}
